                ...
                if (bookEntry.getEntryText().size() > i) {
                    ...
                    for (BookLink bookLink : bookEntry.getEntryLinks()) {
                        if (bookLink.getLineNumber() == i) {
                            ...
                            font.drawInBatch(component, fontWidth, (i - startReadingAt) * 10, bookLink.isHovered() ? CaveBookScreen.TEXT_LINK_HOVER_COLOR : CaveBookScreen.TEXT_LINK_COLOR, false, poseStack.last().pose(), bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
